...
static void test_accept_key_vector() {
    char accept[32];
    const auto result = signalk_mini::websocket::make_accept_key("dGhlIHNhbXBsZSBub25jZQ==", accept, sizeof(accept));
    REQUIRE(result == signalk_mini::websocket::HandshakeResult::Ok);
...
static void test_parse_upgrade_request() {
    ...
    signalk_mini::websocket::HandshakeRequest parsed;
    const auto result = signalk_mini::websocket::parse_client_handshake(request, std::strlen(request), parsed);
    REQUIRE(result == signalk_mini::websocket::HandshakeResult::Ok);
...
static void test_bad_upgrade_requests() {
    const char missing_key[] =
        ...
        "\r\n";
    signalk_mini::websocket::HandshakeRequest parsed;
    REQUIRE(signalk_mini::websocket::parse_client_handshake(missing_key, std::strlen(missing_key), parsed) == signalk_mini::websocket::HandshakeResult::MissingKey);
    ...
    const char wrong_version[] =
        ...
        "\r\n";
    REQUIRE(signalk_mini::websocket::parse_client_handshake(wrong_version, std::strlen(wrong_version), parsed) == signalk_mini::websocket::HandshakeResult::UnsupportedVersion);
}

static void test_write_handshake_response() {
    char response[192];
    const auto result = signalk_mini::websocket::write_server_handshake_response("dGhlIHNhbXBsZSBub25jZQ==", response, sizeof(response));
    REQUIRE(result == signalk_mini::websocket::HandshakeResult::Ok);
...
static void test_decode_masked_client_text() {
    ...
    uint8_t decoded[32];
    signalk_mini::websocket::FrameView view;
    size_t consumed = 0;
    const auto result = signalk_mini::websocket::decode_frame(frame, 6 + std::strlen(payload), decoded, sizeof(decoded), view, consumed, true);
    REQUIRE(result == signalk_mini::websocket::FrameDecodeResult::Ok);
    REQUIRE(consumed == 6 + std::strlen(payload));
    REQUIRE(view.fin);
    REQUIRE(view.opcode == signalk_mini::websocket::Opcode::Text);
...
static void test_encode_server_text() {
    ...
    uint8_t frame[64];
    size_t len = 0;
    REQUIRE(signalk_mini::websocket::encode_text_frame(text, std::strlen(text), frame, sizeof(frame), len));
...
static void test_decode_ping_and_close() {
    ...
    signalk_mini::websocket::FrameView view;
    size_t consumed = 0;
    REQUIRE(signalk_mini::websocket::decode_frame(ping, sizeof(ping), decoded, sizeof(decoded), view, consumed, true) == signalk_mini::websocket::FrameDecodeResult::Ok);
    REQUIRE(view.opcode == signalk_mini::websocket::Opcode::Ping);
    ...
    REQUIRE(signalk_mini::websocket::decode_frame(fragmented_close, sizeof(fragmented_close), decoded, sizeof(decoded), view, consumed, true) == signalk_mini::websocket::FrameDecodeResult::ProtocolError);
}
...
static void test_reject_unmasked_client_frame_and_oversized_payload() {
    ...
    signalk_mini::websocket::FrameView view;
    size_t consumed = 0;
    REQUIRE(signalk_mini::websocket::decode_frame(unmasked, sizeof(unmasked), decoded, sizeof(decoded), view, consumed, true) == signalk_mini::websocket::FrameDecodeResult::ProtocolError);

    const uint8_t too_large[] = {0x81, 0x80 | 126, 0x00, 0x08, 1, 2, 3, 4, 'x', 'x', 'x', 'x', 'x', 'x', 'x', 'x'};
    REQUIRE(signalk_mini::websocket::decode_frame(too_large, sizeof(too_large), decoded, sizeof(decoded), view, consumed, true) == signalk_mini::websocket::FrameDecodeResult::PayloadTooLarge);
}
