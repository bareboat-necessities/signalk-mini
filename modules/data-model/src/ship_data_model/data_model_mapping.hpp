...
inline bool looks_like_pypilot_runtime_name(const char* name) {
    return data_model_name_starts_with(name, "ap.") ||
           ...
           data_model_name_starts_with(name, "gnss.") ||
           ...
           data_model_name_starts_with(name, "sea.") ||
...
inline bool apply_number(DataModel<Real>& model, FieldId id, Real value, uint64_t now_us) {
    switch (id) {
    case FieldId::server_uptime_s: model.autopilot.server.uptime_s.set(value, now_us); return true;
    case FieldId::ap_heading_deg: model.autopilot.controller.heading_deg.set(value, now_us); return true;
    case FieldId::ap_tack_progress_0_1: model.autopilot.tack.progress_0_1.set(value, now_us); return true;
    case FieldId::wind_filtered_direction_deg: model.wind.apparent.filtered_direction_deg.set(value, now_us); return true;
    case FieldId::servo_current_a: model.steering.servo.current_a.set(value, now_us); return true;
...
inline bool read_number(const DataModel<Real>& model, FieldId id, Real& out) {
    switch (id) {
    case FieldId::server_uptime_s: out = model.autopilot.server.uptime_s.value; return model.autopilot.server.uptime_s.valid;
...
inline bool apply_bool(DataModel<Real>& model, FieldId id, bool value, uint64_t) {
    if (id == FieldId::ap_enabled) { model.autopilot.controller.enabled.value = value; return true; }
    return false;
}

template<typename Real>
inline bool read_bool(const DataModel<Real>& model, FieldId id, bool& out) {
    if (id == FieldId::ap_enabled) { out = model.autopilot.controller.enabled.value; return true; }
    return false;
}

template<typename Real>
inline bool apply_mode(DataModel<Real>& model, FieldId id, AutopilotMode value, uint64_t) {
    if (id == FieldId::ap_mode) { model.autopilot.controller.mode.value = value; return true; }
    return false;
}
...
inline bool apply_pilot(DataModel<Real>& model, FieldId id, PilotName value, uint64_t) {
    if (id == FieldId::ap_pilot) { model.autopilot.controller.pilot.value = value; return true; }
    return false;
}

template<typename Real>
inline bool apply_rudder_calibration_state(DataModel<Real>& model, FieldId id, RudderCalibrationState value, uint64_t) {
    if (id == FieldId::rudder_calibration_state) { model.steering.rudder.calibration_state.value = value; return true; }
    return false;
}

template<typename Real>
inline bool apply_string(DataModel<Real>& model, FieldId id, const char* value, uint64_t) {
    if (id == FieldId::server_version) { copy_data_text(model.autopilot.server.version, sizeof(model.autopilot.server.version), value); return true; }
    if (id == FieldId::status_last_error) { copy_data_text(model.autopilot.status.last_error, sizeof(model.autopilot.status.last_error), value); return true; }
    return false;
}

template<typename Real>
inline bool read_string(const DataModel<Real>& model, FieldId id, const char*& out) {
    if (id == FieldId::server_version) { out = model.autopilot.server.version; return true; }
    if (id == FieldId::status_last_error) { out = model.autopilot.status.last_error; return true; }
    return false;
}
