...
bool apply_hfb(const NmeaSentence& sentence, Model& model, uint64_t now_us, ship_data_model::SensorSource source) {
    ...
    float value = 0.0f;
    if (parse_real(sentence.field(0), value)) trawl.headrope_to_footrope_m.set(static_cast<Real>(value), now_us);
    if (parse_real(sentence.field(2), value)) trawl.headrope_to_bottom_m.set(static_cast<Real>(value), now_us);
