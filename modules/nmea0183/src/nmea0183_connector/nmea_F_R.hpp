...
// Balanced NMEA sentence group: F-R.
...
bool apply_rmb(const NmeaSentence& sentence, Model& model, uint64_t now_us, ship_data_model::SensorSource source) {
    ...
    if (parse_lat_lon(sentence.field(5), sentence.field(6), v)) {
        model.route.rmb.destination_lat_deg.set(static_cast<Real>(v), now_us);
    }
    if (parse_lat_lon(sentence.field(7), sentence.field(8), v)) {
        model.route.rmb.destination_lon_deg.set(static_cast<Real>(v), now_us);
    }
    if (parse_real(sentence.field(9), v)) {
        model.route.rmb.range_nmi.set(static_cast<Real>(v), now_us);
    }
    ...
    if (parse_real(sentence.field(11), v)) {
...
bool apply_rlm(const NmeaSentence& sentence, Model& model, uint64_t now_us, ship_data_model::SensorSource source) {
    ...
    float v = 0.0f;
    nmea_copy_span(model.comm.return_link_message.beacon_id,
                   sizeof(model.comm.return_link_message.beacon_id),
                   sentence.field(0));
    if (parse_utc_time_of_day_s(sentence.field(1), v)) {
        model.comm.return_link_message.reception_time_s.set(static_cast<Real>(v), now_us);
    }
    model.comm.return_link_message.message_code = sentence.field(2)[0];
    nmea_copy_span(model.comm.return_link_message.message_body,
                   sizeof(model.comm.return_link_message.message_body),
                   sentence.field(3));
...
bool apply_rpm(const NmeaSentence& sentence, Model& model, uint64_t now_us, ship_data_model::SensorSource source) {
    ...
    if (parse_int32(sentence.field(1), n)) {
        model.propulsion.revolutions.number.set(n, now_us);
    }
    if (parse_real(sentence.field(2), v)) {
        model.propulsion.revolutions.speed_rpm.set(static_cast<Real>(v), now_us);
    }
    if (parse_real(sentence.field(3), v)) {
        model.propulsion.revolutions.propeller_pitch_percent.set(static_cast<Real>(v), now_us);
    }
...
bool apply_rsd(const NmeaSentence& sentence, Model& model, uint64_t now_us, ship_data_model::SensorSource source) {
    ...
    if (parse_distance_nmi(sentence.field(0), sentence.field(11), v)) {
        model.nav.radar_system.origin_range_nmi[0].set(static_cast<Real>(v), now_us);
    }
    if (parse_real(sentence.field(1), v)) {
        model.nav.radar_system.origin_bearing_deg[0].set(static_cast<Real>(wrap_360_deg(v)), now_us);
    }
    if (parse_distance_nmi(sentence.field(2), sentence.field(11), v)) {
        model.nav.radar_system.variable_range_marker_nmi[0].set(static_cast<Real>(v), now_us);
    }
    if (parse_real(sentence.field(3), v)) {
        model.nav.radar_system.electronic_bearing_line_deg[0].set(static_cast<Real>(wrap_360_deg(v)), now_us);
    }
    if (parse_distance_nmi(sentence.field(4), sentence.field(11), v)) {
        model.nav.radar_system.origin_range_nmi[1].set(static_cast<Real>(v), now_us);
    }
    if (parse_real(sentence.field(5), v)) {
        model.nav.radar_system.origin_bearing_deg[1].set(static_cast<Real>(wrap_360_deg(v)), now_us);
    }
    if (parse_distance_nmi(sentence.field(6), sentence.field(11), v)) {
        model.nav.radar_system.variable_range_marker_nmi[1].set(static_cast<Real>(v), now_us);
    }
    if (parse_real(sentence.field(7), v)) {
        model.nav.radar_system.electronic_bearing_line_deg[1].set(static_cast<Real>(wrap_360_deg(v)), now_us);
    }
    if (parse_distance_nmi(sentence.field(8), sentence.field(11), v)) {
        model.nav.radar_system.cursor_range_nmi.set(static_cast<Real>(v), now_us);
    }
    if (parse_real(sentence.field(9), v)) {
        model.nav.radar_system.cursor_bearing_deg.set(static_cast<Real>(wrap_360_deg(v)), now_us);
    }
    if (parse_distance_nmi(sentence.field(10), sentence.field(11), v)) {
        model.nav.radar_system.range_scale_nmi.set(static_cast<Real>(v), now_us);
    }
...
bool apply_rte(const NmeaSentence& sentence, Model& model, uint64_t now_us, ship_data_model::SensorSource source) {
    ...
    if (parse_int32(sentence.field(0), n)) {
        model.route.active.total_messages.set(n, now_us);
    }
    if (parse_int32(sentence.field(1), n)) {
        model.route.active.message_number.set(n, now_us);
    }
    ...
    for (uint8_t i = 0; i < count; ++i) {
        nmea_copy_span(model.route.active.waypoint_id[i],
                       sizeof(model.route.active.waypoint_id[i]),
                       sentence.field(static_cast<uint8_t>(3 + i)));
    }
    model.route.active.waypoint_count.set(static_cast<int32_t>(count), now_us);
    set_source(model.route.active.source, source);
    model.route.active.last_update_us = now_us;
