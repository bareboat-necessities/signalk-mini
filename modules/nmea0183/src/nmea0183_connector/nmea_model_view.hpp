...
struct NmeaGnssAlmanacView {
    ...
    explicit NmeaGnssAlmanacView(ship_data_model::GnssAlmanacData<Real>& data)
        : source(data.source),
...
template<typename Real>
struct NmeaNavigationView {
    ship_data_model::GnssData<Real>& gps;
    NmeaGnssAlmanacView<Real> gps_almanac;
    ...
    explicit NmeaNavigationView(ship_data_model::DataModel<Real>& model)
        : gps(model.gnss.fix),
...
struct NmeaSeaEnvView {
    ...
    ship_data_model::Stamped<int32_t>* trawl_catch_sensor_status;
    ...
    explicit NmeaSeaEnvView(ship_data_model::DataModel<Real>& model)
...
template<typename Real>
struct NmeaDecodeModelView {
    NmeaNavigationView<Real> navigation;
    NmeaSeaEnvView<Real> water;
    ship_data_model::WindData<Real>& wind;
    ship_data_model::ShipImuData<Real>& imu;
    ship_data_model::RudderData<Real>& rudder;

    explicit NmeaDecodeModelView(ship_data_model::DataModel<Real>& model)
