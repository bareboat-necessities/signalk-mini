...
bool apply_cur(const NmeaSentence& sentence,
    ...
    if (parse_real(sentence.field(1), value)) {
        if (sentence.field(2)[0] == 'M') model.sea.current_direction_magnetic_deg.set(static_cast<Real>(wrap_360_deg(value)), now_us);
        else model.sea.current_direction_deg.set(static_cast<Real>(wrap_360_deg(value)), now_us);
    }
...
bool apply_dbt(const NmeaSentence& sentence,
               ...
               ship_data_model::SensorSource source) {
    float depth_m = 0.0f;
    if (!parse_depth_m_from_triplet(sentence, depth_m)) { last_error_ = "bad DBT"; return false; }
...
bool apply_depth_below_keel(const NmeaSentence& sentence,
                            ...
                            ship_data_model::SensorSource source) {
    float depth_m = 0.0f;
    if (!parse_depth_m_from_triplet(sentence, depth_m)) { last_error_ = "bad DBK"; return false; }
...
bool apply_depth_below_surface(const NmeaSentence& sentence,
                               ...
                               ship_data_model::SensorSource source) {
    float depth_m = 0.0f;
    if (!parse_depth_m_from_triplet(sentence, depth_m)) { last_error_ = "bad DBS"; return false; }
...
bool apply_dcn(const NmeaSentence& sentence,
    ...
    if (parse_real(sentence.field(2), value)) model.nav.decca.red_line_of_position.set(static_cast<Real>(value), now_us);
    ...
    if (parse_real(sentence.field(5), value)) model.nav.decca.green_line_of_position.set(static_cast<Real>(value), now_us);
    ...
    if (parse_real(sentence.field(8), value)) model.nav.decca.purple_line_of_position.set(static_cast<Real>(value), now_us);
...
bool apply_dpt(const NmeaSentence& sentence,
    ...
    float depth_m = 0.0f;
    float offset_m = 0.0f;
    if (!parse_real(sentence.field(0), depth_m)) { last_error_ = "bad DPT"; return false; }
    model.sea.depth_m.set(static_cast<Real>(depth_m), now_us);
    if (parse_real(sentence.field(1), offset_m)) model.sea.depth_offset_m.set(static_cast<Real>(offset_m), now_us);
    set_source(model.sea.depth_source, source);
    model.sea.last_update_us = now_us;
    return true;
}
...
bool apply_etl(const NmeaSentence& sentence,
    ...
    if (sentence.field_count > 1) nmea_copy_span(event_log.event_type, sizeof(event_log.event_type), sentence.field(1));
    if (sentence.field_count > 2) event_log.event_state = sentence.field(2)[0];
    if (sentence.field_count > 3) nmea_copy_span(event_log.event_text, sizeof(event_log.event_text), sentence.field(3));
    else if (sentence.field_count > 2) nmea_copy_span(event_log.event_text, sizeof(event_log.event_text), sentence.field(2));
...
bool apply_eve(const NmeaSentence& sentence,
               ...
               uint64_t now_us,
               ship_data_model::SensorSource source) {
    if (sentence.field_count < 1) { last_error_ = "short EVE"; return false; }
    auto& event = model.notifications.messages.event;
    nmea_copy_span(event.event_id, sizeof(event.event_id), sentence.field(0));
    if (sentence.field_count > 1) nmea_copy_span(event.event_source, sizeof(event.event_source), sentence.field(1));
    if (sentence.field_count > 2) event.event_state = sentence.field(2)[0];
    if (sentence.field_count > 3) nmea_copy_span(event.event_text, sizeof(event.event_text), sentence.field(3));
    else if (sentence.field_count > 2) nmea_copy_span(event.event_text, sizeof(event.event_text), sentence.field(2));
