...
bool apply_oln(const NmeaSentence& sentence, Model& model, uint64_t now_us, ship_data_model::SensorSource source) {
    ...
    for (uint8_t i = 0; i < count; ++i) {
        nmea_copy_span(model.nav.omega_lane_numbers.pair[i], sizeof(model.nav.omega_lane_numbers.pair[i]), sentence.field(i));
    }
...
bool apply_r00(const NmeaSentence& sentence, Model& model, uint64_t now_us, ship_data_model::SensorSource source) {
    ...
    for (uint8_t i = 0; i < count; ++i) {
        nmea_copy_span(model.route.active.waypoint_id[i], sizeof(model.route.active.waypoint_id[i]), sentence.field(i));
    }
...
bool apply_rsa(const NmeaSentence& sentence, Model& model, uint64_t now_us, ship_data_model::SensorSource source) {
    float v = 0.0f;
    if (sentence.field_count >= 2 && sentence.field(1)[0] == 'A' && parse_real(sentence.field(0), v)) model.steering.rudder.angle_deg.set(static_cast<Real>(-v), now_us);
    else if (sentence.field_count >= 4 && sentence.field(3)[0] == 'A' && parse_real(sentence.field(2), v)) model.steering.rudder.angle_deg.set(static_cast<Real>(-v), now_us);
...
bool apply_trf(const NmeaSentence& sentence, Model& model, uint64_t now_us, ship_data_model::SensorSource source) {
    ...
    if (parse_lat_lon(sentence.field(2), sentence.field(3), value)) model.nav.transit_fix.latitude_deg.set(static_cast<Real>(value), now_us);
    if (parse_lat_lon(sentence.field(4), sentence.field(5), value)) model.nav.transit_fix.longitude_deg.set(static_cast<Real>(value), now_us);
    if (parse_real(sentence.field(6), value)) model.nav.transit_fix.elevation_angle_deg.set(static_cast<Real>(value), now_us);
    if (parse_int32(sentence.field(7), integer_value)) model.nav.transit_fix.iterations.set(integer_value, now_us);
