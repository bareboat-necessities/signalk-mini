...
bool apply_r00(const NmeaSentence& sentence, Model& model, uint64_t now_us, ship_data_model::SensorSource source) {
    ...
    set_source(model.route.active.source, source); model.route.active.last_update_us = now_us;
    return true;
}
...
bool apply_rma(const NmeaSentence& sentence, Model& model, uint64_t now_us, ship_data_model::SensorSource source) {
    ...
    float v = 0.0f; model.nav.rma.status = sentence.field(0)[0];
...
bool apply_rmb(const NmeaSentence& sentence, Model& model, uint64_t now_us, ship_data_model::SensorSource source) {
    ...
    set_source(model.route.rmb.source, source); set_source(model.route.apb.source, source); model.route.rmb.last_update_us = now_us; model.route.apb.last_update_us = now_us;
    return true;
}
...
template<typename Model>
bool apply_sfi(const NmeaSentence& sentence, Model& model, uint64_t now_us, ship_data_model::SensorSource source) { return apply_notification_text_record(sentence, model.notifications.messages.device_status, now_us, source); }

template<typename Model>
bool apply_stn(const NmeaSentence& sentence, Model& model, uint64_t now_us, ship_data_model::SensorSource source) { int32_t n = 0; if (sentence.field_count < 1 || !parse_int32(sentence.field(0), n)) { last_error_ = "bad STN"; return false; } model.nav.multiple_data_id.talker_id_number.set(n, now_us); set_source(model.nav.multiple_data_id.source, source); model.nav.multiple_data_id.last_update_us = now_us; return true; }

template<typename Model>
bool apply_tds(const NmeaSentence& sentence, Model& model, uint64_t now_us, ship_data_model::SensorSource source) { if (sentence.field_count < 6) { last_error_ = "short TDS"; return false; } float v = 0.0f; if (sentence.field(1)[0] == 'M' && parse_real(sentence.field(0), v)) model.sea.trawl_door_centerline_offset_m.set(static_cast<Real>(v), now_us); if (sentence.field(3)[0] == 'M' && parse_real(sentence.field(2), v)) model.sea.trawl_door_along_centerline_m.set(static_cast<Real>(v), now_us); if (sentence.field(5)[0] == 'M' && parse_real(sentence.field(4), v)) model.sea.trawl_depth_below_surface_m.set(static_cast<Real>(v), now_us); set_source(model.sea.source, source); model.sea.last_update_us = now_us; return true; }

template<typename Model>
bool apply_tfi(const NmeaSentence& sentence, Model& model, uint64_t now_us, ship_data_model::SensorSource source) { if (sentence.field_count < 3) { last_error_ = "short TFI"; return false; } int32_t n = 0; for (uint8_t i = 0; i < 3; ++i) if (parse_int32(sentence.field(i), n)) model.sea.trawl_catch_sensor_status[i].set(n, now_us); set_source(model.sea.source, source); model.sea.last_update_us = now_us; return true; }

template<typename Model>
bool apply_tlb(const NmeaSentence& sentence, Model& model, uint64_t now_us, ship_data_model::SensorSource source) { return apply_notification_text_record(sentence, model.notifications.messages.device_status, now_us, source); }

template<typename Model>
bool apply_tll(const NmeaSentence& sentence, Model& model, uint64_t now_us, ship_data_model::SensorSource source) { if (sentence.field_count < 9) { last_error_ = "short TLL"; return false; } int32_t n = 0; float v = 0.0f; if (parse_int32(sentence.field(0), n)) model.ais.tracked_target.target_number.set(n, now_us); if (parse_lat_lon(sentence.field(1), sentence.field(2), v)) model.ais.tracked_target.latitude_deg.set(static_cast<Real>(v), now_us); if (parse_lat_lon(sentence.field(3), sentence.field(4), v)) model.ais.tracked_target.longitude_deg.set(static_cast<Real>(v), now_us); nmea_copy_span(model.ais.tracked_target.target_name, sizeof(model.ais.tracked_target.target_name), sentence.field(5)); if (parse_utc_time_of_day_s(sentence.field(6), v)) model.ais.tracked_target.utc_time_s.set(static_cast<Real>(v), now_us); model.ais.tracked_target.target_status = sentence.field(7)[0]; model.ais.tracked_target.reference_target = sentence.field(8)[0]; set_source(model.ais.tracked_target.source, source); model.ais.tracked_target.last_update_us = now_us; return true; }
...
template<typename Model>
bool apply_trf(const NmeaSentence& sentence, Model& model, uint64_t now_us, ship_data_model::SensorSource source) { if (sentence.field_count < 12) { last_error_ = "short TRF"; return false; } float v = 0.0f; if (parse_utc_time_of_day_s(sentence.field(0), v)) model.nav.transit_fix.utc_time_s.set(static_cast<Real>(v), now_us); nmea_copy_span(model.nav.transit_fix.date_ddmmyy, sizeof(model.nav.transit_fix.date_ddmmyy), sentence.field(1)); if (parse_lat_lon(sentence.field(2), sentence.field(3), v)) model.nav.transit_fix.latitude_deg.set(static_cast<Real>(v), now_us); if (parse_lat_lon(sentence.field(4), sentence.field(5), v)) model.nav.transit_fix.longitude_deg.set(static_cast<Real>(v), now_us); model.nav.transit_fix.data_validity = sentence.field(11)[0]; set_source(model.nav.transit_fix.source, source); model.nav.transit_fix.last_update_us = now_us; return true; }

template<typename Model>
bool apply_ttm(const NmeaSentence& sentence, Model& model, uint64_t now_us, ship_data_model::SensorSource source) { return apply_notification_text_record(sentence, model.notifications.messages.safety, now_us, source); }

template<typename Model>
bool apply_txt(const NmeaSentence& sentence, Model& model, uint64_t now_us, ship_data_model::SensorSource source) { return apply_notification_text_record(sentence, model.notifications.messages.text, now_us, source); }
...
template<typename Model>
bool apply_vtg(const NmeaSentence& sentence, Model& model, uint64_t now_us, ship_data_model::SensorSource source) { if (sentence.field_count < 7) { last_error_ = "short VTG"; return false; } float v = 0.0f; if (parse_real(sentence.field(0), v)) model.gnss.fix.track_deg.set(static_cast<Real>(wrap_360_deg(v)), now_us); if (parse_knots(sentence.field(4), sentence.field(5), v) || parse_knots(sentence.field(6), sentence.field(7), v)) model.gnss.fix.speed_kn.set(static_cast<Real>(v), now_us); set_source(model.gnss.fix.source, source); model.gnss.fix.last_update_us = now_us; return true; }
...
template<typename Model>
bool apply_wdr(const NmeaSentence& sentence, Model& model, uint64_t now_us, ship_data_model::SensorSource source) { if (sentence.field_count < 6) { last_error_ = "short WDR"; return false; } float v = 0.0f; if (parse_distance_nmi(sentence.field(0), sentence.field(1), v) || parse_distance_nmi(sentence.field(2), sentence.field(3), v)) model.route.waypoint.distance_nmi.set(static_cast<Real>(v), now_us); nmea_copy_span(model.route.waypoint.to_waypoint_id, sizeof(model.route.waypoint.to_waypoint_id), sentence.field(4)); nmea_copy_span(model.route.waypoint.from_waypoint_id, sizeof(model.route.waypoint.from_waypoint_id), sentence.field(5)); set_source(model.route.waypoint.source, source); model.route.waypoint.last_update_us = now_us; return true; }
...
template<typename Model>
bool apply_zda(const NmeaSentence& sentence, Model& model, uint64_t now_us, ship_data_model::SensorSource source) { if (sentence.field_count < 6) { last_error_ = "short ZDA"; return false; } float v = 0.0f; int32_t n = 0; if (!parse_utc_time_of_day_s(sentence.field(0), v)) { last_error_ = "bad ZDA"; return false; } model.gnss.fix.timestamp_s.set(static_cast<Real>(v), now_us); if (parse_int32(sentence.field(1), n)) model.gnss.fix.date_day.set(n, now_us); if (parse_int32(sentence.field(2), n)) model.gnss.fix.date_month.set(n, now_us); if (parse_int32(sentence.field(3), n)) model.gnss.fix.date_year.set(n, now_us); if (parse_int32(sentence.field(4), n)) model.gnss.fix.local_zone_hours.set(n, now_us); if (parse_int32(sentence.field(5), n)) model.gnss.fix.local_zone_minutes.set(n, now_us); set_source(model.gnss.fix.source, source); model.gnss.fix.last_update_us = now_us; return true; }
