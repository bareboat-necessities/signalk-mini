...
bool apply_gsv(const NmeaSentence& sentence, Model& model, uint64_t now_us, ship_data_model::SensorSource source) {
    ...
    copy_gnss_talker_id(rec.talker_id, sizeof(rec.talker_id), sentence);
    if (parse_int32(sentence.field(0), integer_value)) rec.total_messages.set(integer_value, now_us);
    if (parse_int32(sentence.field(1), integer_value)) rec.message_number.set(integer_value, now_us);
    if (parse_int32(sentence.field(2), integer_value)) rec.satellites_in_view.set(integer_value, now_us);
    ...
    if (used_fields < sentence.field_count && parse_int32(sentence.field(used_fields), integer_value)) rec.signal_id.set(integer_value, now_us);
    if (used_fields + 1 < sentence.field_count && parse_int32(sentence.field(static_cast<uint8_t>(used_fields + 1)), integer_value)) rec.system_id.set(integer_value, now_us);
    set_source(rec.source, source); rec.last_update_us = now_us;
    return true;
}
