...
#undef NMEA_APPLY_NO_SOURCE
        if (sentence.family == NmeaSentenceFamily::SeaTalk) {
            return apply_seatalk_tunnel(sentence, model, now_us, source);
        }
...
private:
    ...
    bool apply_seatalk_tunnel(const NmeaSentence& sentence,
                              Model& model,
                              uint64_t now_us,
                              ship_data_model::SensorSource source) {
        ...
        if (sentence.fragment.is_fragmented) {
            ...
            if (state_.seatalk_message.overflow) { last_error_ = "SeaTalk tunnel overflow"; return false; }
            if (!parse_seatalk_payload_text(state_.seatalk_message.text, bytes, count, sizeof(bytes))) { last_error_ = "bad SeaTalk tunnel payload"; return false; }
        ...
        if (!seatalk_receiver_.accept_datagram(bytes, count, model, now_us, source)) {
            last_error_ = "bad SeaTalk tunnel frame";
            return false;
        }
        return true;
    }
